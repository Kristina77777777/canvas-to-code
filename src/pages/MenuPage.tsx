...
import { menuItems, cafeInfo } from '@/data/menuData';
...
const MenuPage: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'non-coffee', label: 'Без кофе', description: 'Для тех, кто не пьёт кофе или хочет что-то другое.' },
    { id: 'seasonal', label: 'Сезонные', description: 'Меняются в зависимости от времени года. Обычно это тёплые напитки зимой и освежающие — летом.' },
    { id: 'waffles', label: 'Вафли', description: 'Готовим вафли на заказ. Подаём горячими. Можно выбрать начинку. Многие гости приходят к нам именно за ними — и возвращаются снова.' },
    { id: 'desserts', label: 'Десерты', description: '' },
  ];
            ...
            <TabsList className="flex flex-wrap justify-start mb-6 bg-transparent gap-2 h-auto">
              {categories.map((category) => (
                <TabsTrigger
                  key={category.id}
                  value={category.id}
                  className="font-body px-4 py-2 rounded-full data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all"
                >
                  {category.label}
                </TabsTrigger>
              ))}
              ...
              <div className="space-y-3">
                {filteredItems.map((item) => (
                  ...
                  >
                    ...
                    <span className="font-body text-lg font-medium text-primary ml-4">
                      {item.price} ₽
                    </span>
                  </div>
                ))}
